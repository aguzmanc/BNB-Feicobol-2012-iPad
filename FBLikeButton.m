...
- (id)initWithFrame:(CGRect)frame andUrl:(NSString *)likePage andStyle:(FBLikeButtonStyle)style andColor:(FBLikeButtonColor)color{
	if ((self = [super initWithFrame:frame])) {
		...
		webView_ = [[UIWebView alloc] initWithFrame:CGRectMake(0, 0, frame.size.width, 70)];
		...
		[[[webView_ subviews] lastObject] setScrollEnabled:NO];
...
- (void) configureTextColors{
	...
	NSString *textColor=@"000000";
	NSString *buttonColor=@"5C74C8";
	NSString *linkColor=@"3B5998";
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType {
	...
	if ([[[request URL] absoluteString] rangeOfString:@"login.php"].location!=NSNotFound){
		loginDialog_= [[[FBDialog alloc] init] autorelease];
